  ...
  return (
    <div className="flex flex-col bg-gray-50 min-h-screen font-sans">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {activeTab === 'Home' && (
          <div className="p-8 mt-4 overflow-hidden">
            <div className="text-center mb-10">
              <h1 className="text-4xl font-black text-gray-900 mb-4">BlazeWorks 3D Printing</h1>
              <p className="text-gray-500 max-w-2xl mx-auto">ยินดีต้อนรับสู่บริการพิมพ์ 3 มิติคุณภาพสูง เครื่องพิมพ์ระดับอุตสาหกรรม การันตีความละเอียดและรวดเร็ว</p>
            </div>
            
            {/* Animated Staggered Gallery (Minimal Parallelogram) */}
            <div className="flex flex-wrap justify-center gap-4 md:gap-8 max-w-[1400px] mx-auto py-16 px-4">
               {/* สร้างรูปภาพ 8 กรอบ */}
               {Array.from({ length: 8 }).map((_, idx) => {
                 const itemPage0 = galleryItems[idx];
                 const itemPage1 = galleryItems[idx + 8];
                 const isEven = idx % 2 === 0;
                 const translateY = isEven ? 'translate-y-4 md:translate-y-8' : '-translate-y-4 md:-translate-y-8';
                 ...
                 return (
                   <div key={idx} className={`relative w-40 h-64 md:w-64 md:h-96 -skew-x-[15deg] ${translateY} rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group bg-white shrink-0`}>
                      ...
                      {/* หน้า 0 */}
                      <div className={`absolute inset-[-25%] transition-all duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)] flex items-center justify-center
                      ...
                      >
                         ...
                         <div className="w-full h-full skew-x-[15deg] scale-[1.3]"
                           style={{
                              backgroundImage: `url(${itemPage0.src})`,
                              backgroundSize: 'cover', backgroundPosition: itemPage0.position || 'center'
                           }}
                         />
                         <div className="absolute inset-0 bg-black/5 hover:bg-transparent transition-colors duration-500 skew-x-[15deg] scale-[1.3]"></div>
                      </div>
                          ...
                          <tr key={q.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors text-center text-sm md:text-base">
                            ...
                            <td className="p-4 md:p-5">
                               <span className={`inline-flex items-center justify-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs font-bold ${
                                  q.isPrinting ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'
                               }`}>
        ...
        {activeTab === 'About' && (
           <div className="p-8 md:p-12 max-w-4xl mx-auto mt-10 bg-white rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="prose prose-lg text-gray-600 max-w-none space-y-6">
                ...
                <div className="bg-[#72D1B7]/5 p-8 rounded-2xl mt-10 text-center border border-[#72D1B7]/20 shadow-sm relative overflow-hidden">
                  ...
                  <a href="https://m.me/IceBlazeLAB" target="_blank" rel="noopener noreferrer" className="relative z-10 inline-block bg-[#72D1B7] hover:bg-[#5bb89e] text-white px-10 py-3 rounded-full font-bold transition-all shadow-md hover:shadow-lg hover:-translate-y-1">
                    ไปที่ Messenger
                  </a>
                </div>
              </div>
